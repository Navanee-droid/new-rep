...
def arg_parsing() -> dict:
    ...
    args = parse_args(
        required=['--a'],
        optional=['--t', '--p', '--l', '--o', '--tb', '--t_nbr', '--filter',
    ...
    # Determine effective apply_filter value:
    #   - If t_nbr is Y or a literal value (not N): default filter to N (no filters)
    #   - If t_nbr is N: default filter to Y (apply filters)
    #   - User can always override via explicit --filter argument
    tenant_number_arg = args_dict['tenant_number_arg'].upper()
    user_filter = args.get('apply_filter')
    if user_filter is not None:
        # User explicitly passed --filter, use their value
        args_dict['apply_filter'] = user_filter.upper()
    else:
        # No explicit --filter passed, apply conditional default
        if tenant_number_arg == 'N':
            args_dict['apply_filter'] = 'Y'
        else:
            args_dict['apply_filter'] = 'N'
...
def infer_business_key(
    ...
    is_dtm = 'DTM' in table_name.upper() or 'DAY_TME' in table_name.upper() or 'DAY_TIME' in table_name.upper()
    if is_dtm:
        schema = appl_name
...
def run_validation_for_tenant(
    ...
    apply_filter = arg_dict.get('apply_filter', 'Y').upper()
    ...
    logging.info(f"  Apply filter:  {apply_filter}")
    logging.info("=" * 80)
    ...
    for table_idx, table_name in enumerate(tables, 1):
        ...
        try:
            ...
            if table_meta.is_c2 and apply_filter == 'Y':
            ...
            # When filters are disabled, clear C2 Snowflake filter attributes
            # so that SRC_APPL_NAME / LOG_DATA_SRC_CDE conditions are not added
            if table_meta.is_c2 and apply_filter != 'Y':
                table_meta.has_src_appl_name = False
                table_meta.has_log_data_src_cde = False
                table_meta.has_data_src_cde = False
            ...
            codes = c2_appl_codes if (c2_appl_codes and apply_filter == 'Y') else None
            ...
            def _get_key_columns():
                ...
                # Try exact match, then with/without TB_ prefix
                kc = biz_keys_config.get(tname, [])
                if not kc and tname.startswith('TB_'):
                    kc = biz_keys_config.get(tname[3:], [])
                if not kc:
                    kc = biz_keys_config.get(f'TB_{tname}', [])
                if kc:
    ...
    if arg_dict.get('load_sf_meta'):
        ...
        try:
            ...
            summary.started_at = datetime.fromtimestamp(
                time.time() - (time.perf_counter() - t_upload_start))
            summary.parameters_used = {k: str(v) for k, v in arg_dict.items() if k not in ('sf_cursor',)}

            _ac = infer_registry_app_category(appl_name)
            ...
            for r in all_results:
                ...
                diff_path = os.path.join(
                    output_dir, f"DIFF_{r.table_name}_{tenant_id}_{arg_dict['process_date']}"
                )
            ...
            summary.execution_time_sec = time.perf_counter() - t_upload_start

            # Attach output files to summary
            _run_suffix = f"_{tenant_id}_{arg_dict['process_date']}"
            _all_output_files = (
                _glob.glob(os.path.join(output_dir, '*.txt')) +
                _glob.glob(os.path.join(output_dir, '*.csv'))
            )
...
def _run_date_range(arg_dict, ingest_cfg_dict, orc_conn_dict, date_list):
    """Run validation for each date in the range individually.

    Loops through each date calling run_validation_for_tenant() (same as
    single-date mode), producing full output files per date per tenant.
    DB connections are reused across dates within a tenant.

    Returns (tenants, flat results list).
    """
    sf_conn = open_sf_connection(
        ingest_cfg=ingest_cfg_dict,
        schema=arg_dict['appl_name']
    ...
    try:
        ...
        for tenant_id in tenants:
            ...
            try:
                ...
                for date_idx, process_date in enumerate(date_list, 1):
                    process_date_ts = f'{process_date[0:4]}-{process_date[4:6]}-{process_date[6:8]}'
                    logging.info(f"\n{'#' * 80}")
                    logging.info(f"# DATE {date_idx}/{len(date_list)}: {process_date_ts} "
                                 f"(tenant {tenant_id})")
                    logging.info(f"{'#' * 80}")

                    # Update arg_dict with the current date
                    arg_dict['process_date'] = process_date
                    arg_dict['process_date_ts'] = process_date_ts
                    arg_dict['proc_yr_mth'] = process_date[0:6]
...
def main():
    ...
    if is_multi_date:
        # ---- Range mode: loop through each date individually,
        #      producing full output per date per tenant ----
